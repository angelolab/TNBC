% MibiPlotGroupsForCohortByMixingVec170907
...
%     plotbrowser on;
    %saveas(f,[path,'/Point',num2str(i),'/TIFs3/groups.png']);
end
...
for j = 1:points
    pointNumber = pointsVecS(j);
    axes(ha(j));
    data = images(:,:,:,pointNumber);
    imshow(data);
    title(['Sample ',num2str(pointNumber)]);
end
%saveas(f,[path,'/ByChannel3/groupsByMixing170907.fig']);