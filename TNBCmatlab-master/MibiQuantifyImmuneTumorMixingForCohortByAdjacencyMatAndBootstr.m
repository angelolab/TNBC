...
for i=1:points
    ...
    tumorNum = length(find(currTumorInds));
    immuneNum = length(find(currImmuneInds));
    ...
    parfor j = 1:BootstrapNum
        ...
        randAdjMatCellsTumorImmune = adjMatCells(randTumorInds,randImmuneInds);
        immuneTumorInteractionsB(i,j) = sum(sum(randAdjMatCellsTumorImmune));      
    end
end

for i=1:points
    [muhat(i),sigmahat(i)] = normfit(immuneTumorInteractionsB(i,:));
    z(i) = (muhat(i)-immuneTumorInteractions(i))/sigmahat(i);
end

figure; bar(z);
set(gca,'xTick',[1:points]);
...
% save results
save([pathSegment,'/mixingData.mat'],'immuneTumorInteractionsB','immuneTumorInteractions','muhat','sigmahat','z','pointsVecS','percentMixS');
